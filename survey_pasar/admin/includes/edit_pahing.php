...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM data_barang_pahing WHERE id_barang = $id_barang";
...
?>
        ...
        <form method="post" action="">
            <div class="card">
                <label><?php echo $row['nama_barang']; ?></label><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_edit'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // update data di database
        $sql = "UPDATE data_barang_pahing SET harga_sekarang = $harga_sekarang, selisih = $selisih_baru WHERE id_barang = $id_barang";
        $result = $conn->query($sql);
