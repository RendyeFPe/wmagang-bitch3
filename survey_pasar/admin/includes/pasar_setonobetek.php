<style>
    tr:nth-child(even) {
        background-color: rgba(238, 217, 24, 0.2);
    }
...
{
    global $conn;
    $sql = "UPDATE data_barang_setonobetek SET status_validasi = 'true' WHERE id_barang = $id_barang";
...
$sql = "SELECT * FROM data_barang_setonobetek WHERE status_validasi = 'pending'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td><a id='validasi-button' href='../admin/includes/pasar_setonobetek.php?action=validasi&id=" . $row['id_barang'] . "'>Validasi</a> | <a id='edit-button' href='?action=edit&id=" . $row['id_barang'] . "&source=edit_setonobetek'>Edit</a></td>";
        echo "</tr>";
    }
    echo "</table>";
    echo "</div>";
